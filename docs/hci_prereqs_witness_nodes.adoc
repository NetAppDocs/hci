...
If you are performing an in-place upgrade of an existing Witness Node to Element 12.8 or later, before you upgrade, you must ensure that the VM hardware version on the Witness Node is compatible with ESXi 6.7 (VM hardware version 14) or later depending on your environment. See link:task_hcc_upgrade_management_node.html#upgrade-vm-hardware[Upgrade VM hardware version]. 
====
