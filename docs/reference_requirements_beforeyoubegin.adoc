...
** 1 virtual IP shared between the load balancer machines
** There are two load balancers each having its own assigned IP. 
...
*	Data (iSCSI Storage) network (Total minimum of 111 IP addresses per workload cluster, default is /16 or 65534 IPs)
** 1 IP for each master node (default is 1 master node)
** 1 IP for each of the worker nodes (default is 2 worker nodes)
** You can use the default of /16 or 65534 IPs or create your own range, where you need to assign a minimum of 111 IP addresses (100 reserved for storage nodes, 1 for Storage Virtual IP, and 10 for user clusters)
