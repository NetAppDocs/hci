...
+
NOTE: In two storage or three storage node configurations, the NDE configures the Witness Nodes to use the local datastore on the compute nodes. As a result, your vSphere Client displays two *Datastore usage on disk* error messages. To continue, select the *Reset To Green* link in each error message.
