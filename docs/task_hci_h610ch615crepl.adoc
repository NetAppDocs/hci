...
. Only for H615C. Remove the DIMMs from the chassis. You will insert these DIMMs in the replacement chassis.
. Remove the two power supply units on either side of the chassis.
...
. Place the chassis that you removed on a level surface.
You should package it and return it to NetApp.
. Replace the power supply units.
. Only for H615C. Insert the DIMMs that you removed from the failed chassis, and insert them in the replacement chassis.
