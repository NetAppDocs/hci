...
. Copy the .VIB driver file from your computer to ESXi running on the compute node. The SCP utility is readily available in most Linux distributions, or available as a downloadable utility for all versions of Windows.
+
In the following example for ESXi 6.0, 6.5, and 6.7, the commands assume that the driver is located in the $HOME/NVIDIA/ESX6.x/ directory on the management host:
+
[%header,cols=2*]
|===
|Option
