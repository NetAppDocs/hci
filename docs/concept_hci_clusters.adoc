...
When you scale a three-node storage cluster to a four-node cluster, the cluster gets more resiliency for auto-healing, more capacity and cluster IOPS. In a four-node cluster, the Witness Node is no longer needed to form the cluster quorum.
