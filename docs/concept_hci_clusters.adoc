...
The authoritative cluster is the storage cluster that NetApp Hybrid Cloud Control uses to authenticate users.
...
Many NetApp Hybrid Cloud Control features are designed to work with multiple clusters, but authentication and authorization have limitations. The limitation around authentication and authorization is that the user from the authoritative cluster can execute actions on other clusters tied to NetApp Hybrid Cloud Control even if they are not a user on the other clusters. Before proceeding with managing multiple storage clusters, you should ensure that users defined on the authoritative clusters are defined on all other clusters with the same permissions. You can manage users from the Element software user interface (Element web UI).
