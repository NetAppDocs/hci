...
.. From the REST API UI, select `GET /assets` to get your *storageID*.
.. Select *Try it out*.
.. Select *Execute*.
...
.. Select *GET ​/health-checks​/{healthCheckId}/log* to list the KB article, if specified, for each issue.
.. Select *Try it out*.
.. Enter the health check ID in the parameter field.
.. Select *Execute*.
.. Scroll to the bottom of the response body.
