...
+
NOTE: You must upgrade to the latest management services bundle before upgrading your
...
After the health check completes without errors, the storage cluster is ready to upgrade. See storage node upgrade link:<task_hcc_upgrade_element_healthtools>.html[instructions] to proceed.
...
.. Go to the specific KB article listed for each issue or perform the specified remedy.
.. If a KB is specified, complete the process described in the relevant KB article.
.. After you have resolved cluster issues, click *GET ​/health-checks​/healthCheckId}* again.
