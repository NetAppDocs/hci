...
== Apply an existing QoS policy to a volume
...
. In the leftmost column of the volumes table, select the check box for each volume that needs a QoS policy assigned.
