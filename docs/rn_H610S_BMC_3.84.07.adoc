...
** The Network setting is used to preserve the DHCP assigned BMC IP
+
...
+
The update process can take approx of 4-5 minutes to complete if the BMC firmware file is in the nodes' network; otherwise the process will take longer.
. Verify the firmware version by looking at the version listed on Dashboard > More info.
