...
* `username`:
* `password`
* `package_name`: Use the value you copied in a previous step.
* `version`: Use the value you copied in a previous step.
* `controller_id` (required only for single-node upgrade): Use the value you copied in the previous step.
* `hardware_id` (required only for single- node upgrade): Use the value you copied in the previous step.
* `cluster_id` (required only for cluster-level upgrade): Use the value you copied in the previous step.
