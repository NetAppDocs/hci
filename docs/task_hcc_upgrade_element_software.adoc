...
CAUTION: When upgrade clusters running 11.x using the NetApp Hybrid Cloud Control UI, the UI session times out after two hours, and you will be asked to log in again. The upgrade process continues to run in the background. You can log back in and see the status of the upgrade from the *Upgrades > Storage* tab.
