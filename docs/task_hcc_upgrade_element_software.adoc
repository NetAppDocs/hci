...
|Upload the upgrade file that you downloaded.
|Click image:hcc_browse_icon.png[browse icon] to choose the upgrade file to upload.
...
NOTE: If you are upgrading an H610S node to Element 12.0 or later, the upgrade process involves two phases. The UI displays a message to notify you about the two-phase upgrade process.
...
After you resolve the error, you can return to the page, and click image:hcc_resume_icon.png[resume button].

NOTE: After the upgrade is complete, the *Upgrade Status* changes to *Up to Date* if the cluster was upgraded to the latest Element version available. Otherwise, it shows *Versions Available*.
...
* For NetApp HCI systems, go to the NetApp HCI software https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab[download page] and download the latest storage node image to a device that is accessible to the management node.
* For SolidFire storage systems, go to the Element software https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab[download page] and download the latest storage node image to a device that is accessible to the management node.
...
.. From the response, copy the package ID (`"id"`).
...
b3fc-2c963f66abc1",
"log": https://localhost:442/storage/upgrades/3fa85f64-1111-4562-b3fc-
2c963f66abc1/log
},
...
. After you have resolved cluster issues, run *PUT ​/upgrades/{upgradesId}*.
. Click *Try it out*.
. Enter the upgrade ID from the previous step in *upgradeId*.
...
|You need to pause the upgrade due to unexpected issues or the length of upgrade time.
a|
...
. Click *Execute*.
|===
.. Run the *PUT ​/upgrades/{upgradesId}* API multiple times, as needed, until the process is complete.
+
...
+
The upgrade has finished successfully when the `percent` value is `100` and the `state` indicates `finished` or `complete`.
