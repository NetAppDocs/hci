...
* For Hybrid Cloud Control, the management node in your environment is running version 11.3.

NOTE: If you are updating storage using HealthTools, the management node must be 11.0, 11.1 or later.

* If you are using HealthTools to upgrade, you have the latest version of HealthTools.
* If you are using NetApp Hybrid Cloud Control to upgrade, and your management node is not connected to the internet, you have downloaded the upgrade package from the NetApp Support Site.
...
Using the NetApp Hybrid Cloud Control (HCC) UI, you can upgrade a storage cluster.
...
* <<Upgrade storage cluster when management node has internet connectivity>>
* <<Upgrade storage cluster when management node is not connected to the internet>>
* <<Upgrade H610S node running Element version earlier than 11.8>>

=== Upgrade storage cluster when management node has internet connectivity
NetApp Hybrid Cloud Control downloads the new update packages from the online software repository. You must ensure that TCP port `443` is open for NetApp Hybrid Cloud Control to communicate with the software repository.
...
=== Upgrade storage cluster when management node is not connected to the internet
If your management node does not have external connectivity, you must download the upgrade package from the NetApp Support Site.
...
. Click *Begin Upgrade*. See <<Upgrade status changes>> for information about the various upgrade status changes.

=== Upgrade H610S node running Element version earlier than 11.8
If you are upgrading an H610S node running a version of Element earlier than 11.8 to Element 12.0 or later, the upgrade process involves two phases. The UI displays a message to notify you about the two-phase upgrade process. See <<Upgrade status changes>>.
...
|Unable to Detect
|For dark sites, NetApp Hybrid Cloud Control does not display states.

|Complete with Follow-up
|Only for H610S nodes. After phase 1 of the upgrade process is complete, this state prompts you to perform phase 2 of the upgrade. After you complete phase 2 and acknowledge that you have completed it, the status changes to *Up to Date*.
|===
