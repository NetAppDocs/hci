...
.. If `failedHealthChecks` indicates that there were problems regarding cluster health, do the following:
... Go to the specific KB article listed for each issue or perform the specified remedy.
... If a KB is specified, complete the process described in the relevant KB article.
... After you have resolved cluster issues, run *POST /upgrades* again.
.. Run the upgrade progress API multiple times, as needed, until the process is complete.
+
