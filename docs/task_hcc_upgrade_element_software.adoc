...
* For NetApp Hybrid Cloud Control, the management node in your environment is running version 11.3.
...
* If you are using NetApp Hybrid Cloud Control, you have ensured that TCP port `443` is open so that NetApp Hybrid Cloud Control can communicate with the online software repository to download upgrade packages.
...
TIP: When upgrading clusters running 11.x using the NetApp Hybrid Cloud Control UI, the UI session times out after two hours, and you will be taken to the log in page. Even if the session times out, the upgrade process continues to run in the background. You can log back in and see the status of the upgrade from the *Upgrades > Storage* tab.
...
. Complete the additional steps required (phase 2), and acknowledge in the UI that you have completed phase 2.
...
|Unable to Detect
|NetApp Hybrid Cloud Control shows this status instead of *Versions Available*.
