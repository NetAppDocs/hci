...
The *Upgrade Status* for the cluster changes to *In Progress*, and a progress bar shows the upgrade status. On screen messages also show node-level faults and displays the node ID of each node in the cluster as the upgrade progresses. You can monitor the status of each node using the Element UI or the NetApp Element plug-in for vCenter Server UI.
+
...
+
After you resolve the error, you can come back to the page, and click image:hcc_resume_icon.png[resume button].
+
...
. To download logs, click *Download Upgrade Logs* after the upgrade is complete.
+
