...
A on-screen message alerts you after the file is successfully uploaded and validated.
image:hcc_upload_message.png[On-screen message showing the package upload was successful.]

|Select from the available packages.
|image:hcc_select_package.png[Shows the list of available storage upgrade packages.]
|===

. Click *Begin Upgrade*.
+
The *Upgrade Status* for the cluster changes to *In Progress*, and a progress bar shows the upgrade status. On screen messages also show cluster faults and displays the node ID of each node in the cluster as the upgrade progresses.
+
TIP: You can choose to pause the upgrade. Depending on the state of the upgrade process, the pause operation can succeed or fail. You will see a UI prompt asking you to confirm the pause operation. It can take up to two hours for the upgrade to be completely paused.
[ADD INFO ABOUT PAUSE AND RESUME.]
...
ADD INFO ABOUT RAINY DAY UPGRADE, H610S NUANCES.
