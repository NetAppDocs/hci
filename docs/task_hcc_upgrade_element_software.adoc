...
If you are upgrading an H610S series node to Element 12.0 or later, the upgrade process involves two phases. Phase 1 which is performed first follows the same steps as the standard Element upgrade process. Phase 2 involves completing steps to perform a complete node shutdown and power disconnect for each H610S node that are described in a required https://kb.netapp.com/app/answers/answer_view/a_id/1103627[KB^].
