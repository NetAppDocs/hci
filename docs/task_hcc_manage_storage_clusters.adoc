...
[NOTE]
====
There is a two-minute polling interval on the storage nodes table to update the table for non-user initiated actions. Before an action, to ensure the you have the most up-to-date status, you can refresh the nodes table by using the refresh icon located on the upper-right side of the nodes table.
