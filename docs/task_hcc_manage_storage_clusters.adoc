...
=== Enable and disable maintenance mode
This link:concept_hci_storage_maintenance_mode.html[maintenance mode] feature options give you the capability to enable and disable maintenance mode for a storage cluster node.
...
+
While *Maintenance Mode* is enabled, maintenance mode actions are unavailable for the selected node and all other nodes on the same cluster. The *Node Status* column displays a wrench icon and a green success banner appears when a node is successfully placed in maintenance mode.
+
...
.Steps
.	For the node under maintenance mode, under *Actions*, select *Disabling Maintenance Mode*.
+
While *Disabling Maintenance Mode* is enabled, maintenance mode actions are unavailable for the selected node and all other nodes on the same cluster.
+
