...
All storage nodes support a network topology that uses four network ports; this topology uses ports A through D on H410S and H610S nodes. Compute nodes support three network topologies:
...
|Option C
4+|Similar to option B, but with native VLANs (or "access ports") on the switch for the management, storage, and vMotion networks
|===
