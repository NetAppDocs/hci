...
* link:task_hcc_upgrade_compute_node_firmware.html[Upgrade the BMC firmware on your node]
...
. Configure your BMC, see link:hci_prereqs_final_prep.html[Configure IPMI for each node] for details.
. See the NetApp Knowledge Base article https://kb.netapp.com/onprem/solidfire/Element_OS/How_to_disable_BMC_web_UI_access_on_HCI_and_SolidFire_H-Series_systems[How to disable BMC web UI access on HCI and SolidFire H-Series systems^] to determine if your BMC web access needs to be disabled. 
...
== Upgrade the firmware on your node
After you replace the compute node, you might have to upgrade the firmware version. See link:task_hcc_upgrade_compute_node_firmware.html[Upgrade compute node firmware] for details.
