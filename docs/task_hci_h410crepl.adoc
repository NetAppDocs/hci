...
NetApp recommends using the NetApp Deployment Engine (NDE) to add a replacement compute node. If you cannot proceed with using the NDE for ESXi installation, see the NetApp Knowledge Base article https://kb.netapp.com/Legacy/NetApp_HCI/OS/How_to_install_ESXi_on_NetApp_HCI_compute_node_manually[How to install ESXi on NetApp HCI compute node manually^].
====
...
<<Step 1: Prepare to replace the compute node>>
<<Step 2: Replace the compute node in the chassis>>
<<Step 3: Remove the compute node asset in NetApp HCI 1.7 and later>>
<<Step 4: Add the compute node to the cluster>>
<<Step 5: Redeploy Witness Nodes for two and three-node storage clusters>>
...
. Log in to the NDE by providing the local NetApp HCI storage cluster administrator credentials. 
+
...
If your NetApp HCI installation runs version 1.4P2, 1.4, or 1.3, you can use the NDE to add the node to the cluster.
...
+
NOTE: When asked to select the disk to install ESXi on, you should select the second disk drive in the list by selecting the Down Arrow key. When asked to enter a root password, you should enter the same password that you configured in the NDE when you set up NetApp HCI.
...
+
NOTE: The password should match what you configured in the NDE when you set up NetApp HCI.
...
. Beginning with Element Plug-in for vCenter 5.0, add the node (host) to the datacenter. For Element Plug-in for vCenter 4.10 and earlier, add the node (host) to the cluster.
+
...
--
.. Beginning with Element Plug-in for vCenter 5.0, in the VMware vSphere Web Client, select *Inventory > Hosts and Clusters*.
...
User name: root
Password: The password you configured in the NDE when you set up NetApp HCI
+
...
--
..  For Element Plug-in for vCenter 4.10 and earlier, in the VMware vSphere Web Client, select *Hosts and Clusters*.
...
The VM is configured using the template that is created as part of initial deployment using the NDE. Here is an example of what a Witness Node VM looks like:
+
