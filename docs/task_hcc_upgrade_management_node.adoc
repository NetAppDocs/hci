...
.	(For persistent volumes configurations only) Copy the contents of the container folder for backup:
+
----
...
.	Move the contents of the container folder back to original location:
+
* Persistent volumes configurations:
+
----
sudo mv /sf/etc/mnode/volumes/* /var/lib/docker/volumes/
----
+
* Configurations without persistent volumes:
+
----
