...
keywords: netapp, hci, on premise, cluster, management node, mNode, upgrade, 10, 11.0, 11.1, 11.3, 11.5, 11.7, 12.0, 12.2, 12.3, 12.3.1
---
...
[.lead]
You can upgrade your management node to management node version 12.3.1 from version 11.0 or later.
...
There are no additional functionality changes or bug fixes, so if you are already running management node 12.3, you do not need to upgrade it to 12.3.1.
Note that if you choose to proceed with an upgrade on a management node 12.3 deployed using NDE, the upgrade to 12.3.1 will complete. However, it might encounter an error during restart. If this occurs, reboot the management node so that it correctly starts on 12.3.1.
* If you are upgrading from management node 12.2:
<<Upgrade a management node to version 12.3.1 from 12.2>>
* If you are upgrading from management node 12.0:
<<Upgrade a management node to version 12.3.1 from 12.0>>
* If you are upgrading from management node 11.3, 11.5, 11.7, or 11.8:
<<Upgrade a management node to version 12.3.1 from 11.3 through 11.8>>
* If you are upgrading from management node 11.0 or 11.1:
<<Upgrade a management node to version 12.3.1 from 11.1 or 11.0>>
...
You can perform an in-place upgrade of the management node from version 12.2 to version 12.3.1 without needing to provision a new management node virtual machine.

NOTE: The Element 12.3.1 management node is an optional upgrade. It is not required for existing deployments.

.What you'll need

* The RAM of the management node VM is 24GB.
* The management node you are intending to upgrade is version 12.0 and uses IPv4 networking. The management node version 12.3.1 does not support IPv6.
+
...
* If you are updating your management node to version 12.3.1, you need management services 2.14.60 or later to proceed.
...
You can perform an in-place upgrade of the management node from version 12.0 to version 12.3.1 without needing to provision a new management node virtual machine.
...
You can perform an in-place upgrade of the management node from version 11.3, 11.5, 11.7, or 11.8 to version 12.3.1 without needing to provision a new management node virtual machine.

NOTE: The Element 12.3.1 management node is an optional upgrade. It is not required for existing deployments.

.What you'll need

* The management node you are intending to upgrade is version 11.3, 11.5, 11.7, or 11.8 and uses IPv4 networking. The management node version 12.3.1 does not support IPv6.
+
TIP: To check the version of your management node, log in to your management node and view the Element version number in the login banner.
...
== Upgrade a management node to version 12.3.1 from 11.1 or 11.0
You can perform an in-place upgrade of the management node from 11.0 or 11.1 to version 12.3.1 without needing to provision a new management node virtual machine.
...
* The management node you are intending to upgrade is version 11.0 or 11.1 and uses IPv4 networking. The management node version 12.3.1 does not support IPv6.
+
TIP: To check the version of your management node, log in to your management node and view the Element version number in the login banner.
...
. On the 12.3.1 management node, run the `upgrade-mnode` script to retain previous configuration settings.
+
...
. (For all NetApp HCI installations and SolidFire all-flash storage installations with NetApp Element Plug-in for vCenter Server) Update the vCenter Plug-in on the 12.3.1 management node by following the steps in the link:task_vcp_upgrade_plugin.html[Upgrade the Element Plug-in for vCenter Server] topic.
