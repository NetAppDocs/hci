...
If you are performing an in-place upgrade of an existing management node to Element 12.8 or later, before you upgrade, you must ensure that the VM hardware version on the management node is compatible with ESXi 6.7 (VM hardware version 14) or later depending on your environment. 
