...
+
NOTE: When running the `sudo /sf/rtfi/bin/sfrtfi_inplace file:///upgrade/casper/filesystem.squashfs sf_upgrade=1` command, the SSH session is killed and console access is required for continued monitoring.
If no console access is available to you when performing the upgrade, retry the SSH login and verify connectivity after 15 to 30 minutes.
Once you login, the new OS version is confirmed in the SSH banner indicating that the upgrade was successful.
+
