...
NOTE: You can find the link:task_mnode_api_get_authorizationtouse.html[bearer] `${TOKEN}` used by the API command using the REST API and retrieving it from the curl response.
