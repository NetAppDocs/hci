...
permalink: docs/rn_storage_firmware_x.xx.xx.html
...
////
= Storage firmware 3.84.07 Release Notes ???
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
== What's New
H610S BMC x.xx.xx contains various bug fixes and new features. This release brings to the field critical updated BMC firmware ahead of subsequent Element software and storage firmware bundle releases, which will also include this updated BMC firmware for H610S nodes.
...
. Choose the BMC update file you donwloaded and click *Start firmware update* to start the upgrade process.
+
