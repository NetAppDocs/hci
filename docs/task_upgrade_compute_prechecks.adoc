...
.. In the payload, enter the `"clusterId"` you copied from the previous step for the `"cluster"` value. You do not need to make any additional modifications to the payload.
+
----
{
  "cluster": "domain-1",
...
+
The code 200 response gives two task IDs:

* A `taskId` that you can use to monitor the task.
* A `"resourceLink"` URL with the task ID needed to confirm the health check results.
----
{
  "resourceLink": "https://10.117.150.84/vcenter/1/compute/tasks/[This is the task ID for task result]",
  "serviceName": "vcenter-v2-svc",
  "taskId": "[This is the task ID for task monitoring]",
  "taskName": "VCenter service health checks"
}
----
.. (Optional) Copy the `taskId` that is part of the response to monitor an active task.
