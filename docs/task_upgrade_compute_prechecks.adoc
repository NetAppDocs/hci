...
.. In the payload, enter the `"clusterId"` you copied from the previous step for the `"cluster"` value:
+
NOTE: You do not need to make any additional modifications to the payload.
...
----
.. (Optional) Copy the `taskId` that is part of response to monitor an active task.
