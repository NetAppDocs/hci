...
+
NOTE: Starting with Element 12.3.1, substeps (a) to (e) are performed automatically. For management node 12.3.1, proceed to substep (f) to complete the time sync configuration. As a result of NTP automation, when you run the setup command, you see link:task_mnode_install.html#auto-ntp[this output] in the setup script.
...
[NOTE]
====
As a result of NTP automation, when you run the setup command, you see the following output in the setup script:
[[auto_ntp]]
----
