...
If your NetApp HCI installation is running Element 12.8, upgrade the VM hardware version on the management node to ESXi 6.7 or later before you upgrade the management node. This ensures that you have a supported ESXi version.

NOTE: You can also follow these steps to upgrade the VM hardware version on the Witness Nodes.
