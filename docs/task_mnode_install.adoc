...
.. (For NetApp HCI only) From the Rest API UI, click *GET ​/installations​/{id}*.
.. Click *Try it out*.
.. Enter the installation ID.
.. Click *Execute*.
.. From the code 200 response body, copy and save the compute node hardware tag (`hardwareTag`) for use in a later step:
+
----
"nodes": [
       {
         "bmcDetails": {
           "bmcAddress": "10.111.11.11",
           "credentialsAvailable": false,
           "credentialsValidated": false
         },
         "chassisSerialNumber": "111921011111",
         "chassisSlot": "B",
         "hardwareId": null,
         "hardwareTag": "00000000-0000-0000-0000-aa1b6cd11e2f"
----
+
NOTE: If your installation is missing the hardware tag value, you can locate the hardware tag for your compute node in vSphere. To do so, select the host and click *Monitor > Hardware Health*. The node BIOS manufacturer and model number are listed. Copy and save the value for `tag` for use in a later step.
