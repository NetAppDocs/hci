...
...	Add a new line for each default time server you want to add. The default time server(s) must be the same NTP server used on the storage cluster. Use the same storage MVIP you specified for `/sf/packages/mnode/setup-mnode`.
+
----
vi /etc/ntp.conf
...
server <insert the name or MVIP of the default time server>
----
