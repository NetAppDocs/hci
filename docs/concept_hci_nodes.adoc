...
As a best practice, you should configure Witness Nodes to use compute node local storage (default set by NDE) and not run on shared storage, such as SolidFire volumes. You should not load the balance with Distributed Resource Scheduler or migrate with vMotion. This might result in both Witness Nodes running on the same compute node and create in a non-high availability (HA) pair configuration.
