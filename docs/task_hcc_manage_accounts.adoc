...
+
NOTE: It might take the NetApp Hybrid Cloud Control UI up to 2 minutes to refresh the inventory. To manually refresh inventory, access the REST API UI inventory service `https://<ManagementNodeIP>/inventory/1/` and run `GET /installations​/{id}` for the cluster.
