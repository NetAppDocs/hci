...
|Management node
|623 UDP
a|Remote Management Control Protocol (RMCP) port. Required for NetApp Hybrid Cloud Control compute firmware upgrades.

NOTE: The port must be open between the management node and the BMC for the storage node.
...
|Compute node BMC/IPMI
|623 UDP
a|RMCP port. Required for NetApp Hybrid Cloud Control compute firmware upgrades.
...
|Compute node BMC/IPMI H410 and H600 series
|623 UDP
.2+a|RMCP port. This is required to manage IPMI-enabled systems.
