...
[.lead]
You might need to allow the following ports through your datacenter's edge firewall so that you can manage the system remotely, allow clients outside of your datacenter to connect to resources, and ensure that internal services can function properly. Some of these URLs or IP addresses might not be required, depending on how you use the system. Some URLs or IP addresses might resolve to other URLs or IP addresses if the server is part of a content delivery network. All ports are TCP unless stated otherwise, and should be open bidirectionally.
...
|Management node
|Online software repository: `repo.netapp.com/api/repos/netapp-downloads`, `repo.netapp.com/bintray/dl/`, `repo.netapp.com/bintray/api/package`, `netapp-downloads.bintray.com`, `bintray.com`, `repo.netapp.com`, `library.netapp.com`
|443
