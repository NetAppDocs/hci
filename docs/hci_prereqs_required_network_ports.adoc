...
NOTE: Some URLs or IP addresses might resolve to other URLs or IP addresses if the server is part of a content delivery network. You might need to work with a network engineer to add these URLs or IP addresses to the firewall rules, using the following general steps:
1. Use the Wget or cURL utility to access a URL (repo.netapp.com, for example)
2. Receive failure to access the URL (different from repo.netapp.com)
3. Add the new URL to the firewall rules
4. Repeat the above steps for this URL until the access attempt succeeds
