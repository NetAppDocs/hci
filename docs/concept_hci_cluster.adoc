...
[.lead]
A cluster is made up of a collection of storage nodes. Starting with NetApp HCI 1.8, you can have a storage cluster with two storage nodes. A cluster appears on the network as a single logical group and can then be accessed as block storage.
