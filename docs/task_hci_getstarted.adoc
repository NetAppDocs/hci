...
* <<Complete tasks after installing hardware (Optional)>>
* <<Deploy NetApp HCI using the NetApp Deployment Engine (NDE)>>
* <<Manage NetApp HCI using the vCenter Plug-in>>
* <<Monitor, upgrade, or expand NetApp HCI with the Hybrid Cloud Control (Optional)>>
...
* For details about configuring Link Aggregation Control Protocol (LACP) on the switch ports used for each of the storage nodes, see link:hci_prereqs_LCAP_configuration.html[Configure LCAP for optimal storage performance].
...
== Complete tasks after installing hardware (Optional, yet recommended)
After installing the NetApp HCI hardware, you should perform some optional tasks
...
=== Configure LACP for optimal storage cluster performance
...
For more information about LACP configuration, see
* https://www.netapp.com/us/media/tr-4820.pdf[_TR-4820: NetApp HCI Networking Quick Planning Guide_]
* link:<https://docs.netapp.com/us-en/hci/docs/hci_prereqs_LACP_configuration.html>[Configure LACP for optimal storage performance]
...
See link:<https://docs.netapp.com/us-en/hci/docs/hci_prereqs_final_prep.html>[Configure IPMI]
...
You can use the NetApp HCI Hybrid Cloud Control to monitor, upgrade, or expand your system.
