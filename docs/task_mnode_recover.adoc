...
. Ensure time is synced between the management node and the storage cluster using NTP or VMware Tools:
...
* VMware Tools
.. Disable NTP on the management node:
+
----
systemctl disable ntp.service
----

.. In vSphere, enable *Synchronize guest time with host* in the VM options.
...
+
----
tar xvf /sf/etc/mnode/[management services bundle file]
----
