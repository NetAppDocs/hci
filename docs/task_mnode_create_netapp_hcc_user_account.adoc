...
[NOTE]
====
* Beginning with NetApp Element Plug-in for vCenter Server 5.0, to use https://docs.netapp.com/us-en/vcp/vcp_concept_linkedmode.html[vCenter Linked Mode^], you register the Element Plug-in from a separate management node for each vCenter Server that will be managing NetApp SolidFire storage clusters. 
