...
NOTE: Before upgrading to version 2.27, make sure you check the current ESXi driver version.  If the driver is out of date, upgrade the driver first. Then upgrade the compute firmware to version 2.27 for your compute nodes. See link:task_hcc_upgrade_compute_node_drivers.html[Update compute node drivers^] for instructions on updating your compute node drivers, which include the ESXi driver.
...
a|If you want to upgrade other firmware on H410C or H300E/H500E/H700E nodes and upgrade Mellanox NIC out-of-band, perform the following steps:

. Upgrade other firmware via compute firmware bundle 12.2.91.
. Upgrade Mellanox firmware via out-of-band methods.
