...
//IMPORTANT: This storage only firmware documentation is presented as a feature preview. Element software bundles will continue to include the storage firmware, but you will soon have the flexibility to update only the firmware using a storage firmware only package when it becomes available.
