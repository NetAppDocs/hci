...
Compute node assets, including vCenter and hardware assets, must be known to management node assets. You can use the inventory service APIs to verify assets:
+
----
...
. After you have resolved node issues, reauthenticate if needed and click *POST /nodes/{hardware_id}/upgrades*.
...
+
During the upgrade, the `status` indicates `running` if no errors are encountered. As each upgrade step finishes, the `status` value changes to `completed`.
+
