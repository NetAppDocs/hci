...
* *Licensing*: SuperMicro licenses for models H300E, H500E, H700E, H410C must be activated to perform an upgrade.
* Close all open SOL sessions before starting Compute upgrade. Upgrade will open SOL session to collect logs from ISO.
...
.. Click *Execute*.
+
IMPORTANT: Do not log in to the BMC. Logging into the BMC terminates the Hybrid Cloud Control Serial-Over-LAN (SOL) session that monitors  upgrade process.
