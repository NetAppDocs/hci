...
== Use a USB drive with the latest firnware image downloaded
...
. On the console, select *Media > Virtual Media Wizard*. [NEED IMAGE]
. Click *Browse* and select the compute firmware .iso file.
. Click *Connect*.
A popup indicating success is displayed, along with the path and device showing at the bottom. You can close the *Virtual Media* window.[need image]
...
. Select AMI Virtual CDROM from the list displayed and click *Enter*. [NEED IMAGE]
...
+
CAUTION: During the powering on process, for approximately 5 minutes, the KVM console displays *No Signal*.
