...
NOTE: Using the NetApp Hybrid Cloud Control UI to upgrade compute firmware on H300E/H500E/H700E compute nodes is no longer supported. To upgrade, it is recommended that you use a <<manaul_method_USB,USB drive>> or the <<manual_method_BMC, BMC UI>> to mount the compute firmware ISO.
