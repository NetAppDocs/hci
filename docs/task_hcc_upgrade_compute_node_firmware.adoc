...
. If the health checks passed without critical failures, you will see *Ready to be Upgraded*. Click *Upgrade Node*.
...
CAUTION: While upgrading the firmware on H610C and H615C compute nodes, do not open the console. This might cause the upgrade to fail.
...
|===
. Locate the compute controller ID for the node you intend to upgrade:
...
.. Click *Execute*.
.. From the response, copy and save the cluster controller ID (`"controllerId"`).
. Using the BMC UI for your node, locate the node hardware ID (BIOS UUID of the hardware) for the node you intend to upgrade using one of the following paths:
