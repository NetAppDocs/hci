...
TIP: For dark site upgrades, you can reduce upload time if the compute firmware bundle and the management node are both local.
...
a|
. Select the cluster you are upgrading.
. Select *Browse* to upload the compute firmware bundle that you downloaded from the
...
. Find the compute firmware bundle ID:
.. From the REST API UI, select *GET /packages*.
.. Select *Try it out*.
.. Select *Execute*.
.. From the response, copy and save the compute firmware bundle name (`"packageName"`) and version (`"packageVersion"`) for use in a later step.
