...
[IMPORTANT]
====
After you upgrade to VCP 4.8 with VMware vCenter Server 6.7U1, you might encounter the following issues:

* The clusters are not listed in the *Clusters* section.
* A server error appears in the *Clusters and QoSSIOC Settings* section of the NetApp Element Configuration.

To resolve the issue, restore the clusters and QoSSIOC settings:

. Log out of vCenter and then log back in.
