...
. Log in to the compute host and verify that it sees all the NetApp HCI datastores. For a typical NetApp HCI system, you should see all the ESXi local datastores and at least the following shared datastores: NetApp-HCI-Datastore-[01,02]
...
. After you power on the virtual machines, wait for approximately 5 minutes and then use a web browser to navigate to the IP address or FQDN of the vCSA applicance. If you do not wait long enough, you will get a message stating that the vSphere Client web server is initializing.
