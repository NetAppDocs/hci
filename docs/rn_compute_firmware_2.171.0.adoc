---
sidebar: sidebar
permalink: docs/rn_compute_firmware_2.171.0.html
...
////
= Compute Firmware Bundle 2.171.0 Release Notes
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
NOTE: NetApp Hybrid Cloud Control requires the latest management services bundle. You must upgrade to the latest management services bundle before upgrading the firmware on your compute nodes to version 2.171.0. If you are upgrading the firmware on your compute nodes to version 2.171.0, you need management services 2.20 to proceed.

== What's New
Version 2.171.0 contains general bugfixes.
...
NOTE: Before upgrading to version 2.171.0, make sure you check the current ESXi driver version. If the driver is out of date, upgrade the driver first. Then upgrade the compute firmware to version 2.171.0 for your compute nodes. Visit link:task_hcc_upgrade_compute_node_drivers.html[Update compute node drivers] for instructions on updating your compute node drivers, which include the ESXi driver.

When you manually install or upgrade host OSes, NetApp cannot control the drivers and firmware for these OSes.

== Download Compute Firmware Bundle 2.171.0
