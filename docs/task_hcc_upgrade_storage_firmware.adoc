...
IMPORTANT: Element software bundles will continue to include the storage firmware, but you will soon have the flexibility to update only the firmware using a storage firmware only package when it becomes available.
...
IMPORTANT: You must upgrade to the latest management services bundle before upgrading the firmware on your storage nodes.  If you are updating your Element software to version 12.2 for SolidFire Enterprise SDS support, you need management services 2.14.60 or later to proceed.
