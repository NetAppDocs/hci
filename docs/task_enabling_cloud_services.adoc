...
+
NetApp Hybrid Cloud Control conducts precheck validations to check your environment. Checks include identifying the installation, the vCenter instance, the vSphere datacenter and cluster, the Cloud Central organization, and the NetApp Kubernetes Service data. If any of the prechecks fail, you are prompted to return to the page in the wizard to change the information.  
