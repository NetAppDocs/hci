...
NOTE: In order to manage a compute node in HCC, you must add the compute node to a vCenter host cluster. The following example displays Node1 and Node2 in a host cluster. Node3 is outside the host cluster, it is displayed in the HCC nodes view but it cannot be managed.
