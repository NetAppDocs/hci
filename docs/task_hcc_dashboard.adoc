...
NOTE: In order to manage a compute node in HCC, you must add the compute node to a vCenter host cluster. In the following example, Node1 and Node2 are in the host cluster where as Node3 is outside the host cluster. In this scenario, Node3 is displayed in the HCC nodes view but cannot be managed.
