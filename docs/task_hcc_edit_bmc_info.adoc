...
[.lead]
You can change Baseboard Management Controller (BMC) administrator credentials in NetApp Hybrid Cloud Control for each of your compute nodes. You might need to change credentials using NetApp Hybrid Cloud Control or the REST API prior to upgrading BMC firmware.
...
NOTE: If you edit BMC information and continue to see error messages like "Hardware ID not available" or "Unable to detect" on the compute firmware upgrade page in the NetApp Hybrid Cloud Control UI, follow the workaround instructions in the Knowledgebase article https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/NetApp_HCI/Known_issues_and_workarounds_for_Compute_Node_upgrades[Known issues and workarounds for compute node upgrades^].
...
.. Click *Execute*.
.. From the response, copy and save the node hardware tag (`hardwareTag`), asset id (`id`), BMC IP address (`bmcAddress`), and node serial number (`chassisSerialNumber`) for use in a later step.
+
----
"nodes": [
  {
...
----
+
NOTE: If your installation is missing the `hardwareTag` value, you can locate the hardware tag for your compute node in vSphere. To do so, select the host and click *Monitor > Hardware Health*. The node BIOS manufacturer and model number are listed. Copy and save the *Tag* value for use in a later step.
...
. Click *Try it out*.
. Enter the node hardware tag (`hardwareTag` from the API response, or *Tag* from vSphere) from the previous step in `hardware_id` field.
. Enter the following parameters in the payload:
+
|===
|Parameter |Description

|`assetId`
|The asset id (`id`) from the previous step.

|`bmcIp`
|The BMC IP address (`bmcAddress`) from the previous step.

|`bmcPassword`
|An updated password to log into BMC.

|`bmcUsername`
|An updated user name to log into BMC.

|`serialNumber`
|The chassis serial number of hardware.
|===
+
Example payload:
+
----
{
  "assetId": "8cd91e3c-1b1e-1111-b00a-4c9c4900b000",
  "bmcIp": "10.117.1.111",
