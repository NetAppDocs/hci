...
----
+
NOTE: If your installation is missing the `hardwareTag` value, you can locate the hardware tag for your compute node in vSphere. To do so, select the host and click *Monitor > Hardware Health*. The node BIOS manufacturer and model number are listed. Copy and save the value for `tag` for use in a later step.
...
. Click *Try it out*.
. Enter the node hardware tag (`hardwareTag`) from the previous step in `hardware_id` field.
