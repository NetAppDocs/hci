...
** Installed the new node in the NetApp HCI chassis by following the  https://docs.netapp.com/us-en/hci/docs/task_hci_installhw.html[installation instructions].
...
+
NOTE: If NetApp HCI cannot recognize the network settings of the cluster you have selected for expansion, ensure that the vmkernel and vmnic mapping for the management, storage and vMotion networks are set to the deployment defaults. See https://docs.netapp.com/us-en/hci/docs/task_nde_supported_net_changes.html[supported networking changes] for more information.
